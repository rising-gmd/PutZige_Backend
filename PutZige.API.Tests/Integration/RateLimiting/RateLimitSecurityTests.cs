...
namespace PutZige.API.Tests.Integration.RateLimiting
{
    public class RateLimitSecurityTests : IntegrationTestBase
    {
        ...
        public async Task Security_BruteForceLogin_BlockedAt5Attempts()
        {
            ...
            var res = await Client.PostAsJsonAsync(TestApiEndpoints.AuthLogin, new LoginRequest { Identifier = email, Password = "Wrong" });
        ...
        public async Task Security_DistributedBruteForce_MultipleIPs_DetectedAndBlocked()
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                await c.PostAsJsonAsync(TestApiEndpoints.AuthLogin, new LoginRequest { Identifier = email, Password = "Wrong" });
            }
        ...
        {
            var exists = await Client.PostAsJsonAsync(TestApiEndpoints.AuthLogin, new LoginRequest { Identifier = "noexist@example.com", Password = "x" });
            var sw = System.Diagnostics.Stopwatch.StartNew();
            var exists2 = await Client.PostAsJsonAsync(TestApiEndpoints.AuthLogin, new LoginRequest { Identifier = "sec2@example.com", Password = "Wrong" });
            sw.Stop();
        ...
        public async Task Security_HeaderInjection_XForwardedFor_Sanitized()
        {
            ...
            {
                Content = JsonContent.Create(new LoginRequest { Identifier = "x@x.com", Password = "x" })
            };
        ...
        {
            var payload = new LoginRequest { Identifier = "' OR 1=1;--@example.com", Password = "x" };
        ...
        public async Task Security_PasswordSpray_AcrossMultipleAccounts_Limited()
        {
            for (int i = 0; i < 20; i++)
            {
                ...
                var r = await Client.PostAsJsonAsync(TestApiEndpoints.AuthLogin, new LoginRequest { Identifier = email, Password = "Password1!" });
            }

            // Ensure no crashes and responses returned
            var final = await Client.PostAsJsonAsync(TestApiEndpoints.AuthLogin, new LoginRequest { Identifier = "spray0@example.com", Password = "Wrong" });
            final.StatusCode.Should().BeOneOf(HttpStatusCode.BadRequest, HttpStatusCode.TooManyRequests);
        }
    }
}
